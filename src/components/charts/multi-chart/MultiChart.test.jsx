...
import MultiChart, { defaultTooltipFormatter } from '.';
...
describe('MultiChart', () => {
  test('verify if created', () => {
    const wrapper = mount(<MultiChart {...defaultProps(numberData())} />);
    expect(wrapper.find(MultiChart)).toHaveLength(1);
  });

  test('verify empty state when all data values equal zero', () => {
    const { container } = render(<MultiChart {...defaultProps(emptyData())} />);
